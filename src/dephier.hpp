...
DepressionHierarchy<elev_t> GetDepressionHierarchy(
    const Array2D<elev_t>& dem,
    const std::vector<double>& cell_area,
    richdem::Array2D<int>& label,
    richdem::Array2D<int>& final_label,
    richdem::Array2D<int8_t>& flowdirs) {
...
#pragma omp parallel for default(none) shared(dem,dx,dy,label,neighbours,progress) private(ocean_seeds) collapse(2) reduction(+:pit_cell_count) reduction(merge:land_seeds)
  ...
  while (!pq.empty()) {
    ...
    if (clabel == OCEAN) {
      ...
      newdep.dep_label = clabel;  // I am storing the label in the object so that I can find it later and call up the
                                  // number of cells and volume
      label(ci) = clabel;         // Update cell with new label

      newdep.my_cells.emplace_back(
          ci);  // because we are going through the priority queue in order from lowest to highest cells,
...
void CalculateMarginalVolumes(
  ...
  for (int y = 0; y < label.height(); y++)
    for (int x = 0; x < label.width(); x++) {
      ...
      total_volumes[clabel] += (deps[clabel].out_elev - static_cast<double>(dem(x, y))) * cell_area[y];
      cell_counts[clabel]++;
  ...
  for (unsigned int i = 0; i < deps.size(); i++) {
    deps[i].dep_area += total_areas[i];  // We need to know the area of our child depressions when getting the total
                                         // depression volumes below.
...
void CalculateTotalVolumes(DepressionHierarchy<elev_t>& deps) {
  ...
  for (int d = 0; d < (int)deps.size(); d++) {
    ...
    if (dep.lchild != NO_VALUE) {
      ...
      // remember to add the area covered by child depression cells, so that our parent can also get the correct total
      // dep_vol.
...
template <class elev_t>
void LastLayer(
    richdem::Array2D<dh_label_t>& label,
    const richdem::Array2D<float>& dem,
