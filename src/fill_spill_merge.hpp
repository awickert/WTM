...
template <class elev_t>
static void MoveWaterIntoPits(Parameters& params, DepressionHierarchy<elev_t>& deps, ArrayPack& arp);
...
static void CalculateWtdVol(
    ...
    const Array2D<dh_label_t>& final_label,
    DepressionHierarchy<elev_t>& deps);

static double CalculateInfiltration(int cell, double distance, double h_0, ArrayPack& arp);
...
template <class elev_t>
static SubtreeDepressionInfo
FindDepressionsToFill(const int current_depression, DepressionHierarchy<elev_t>& deps, ArrayPack& arp);

template <class elev_t>
static void FillAFullDepression(SubtreeDepressionInfo& stdi, DepressionHierarchy<elev_t>& deps, ArrayPack& arp);
...
void FillSpillMerge(Parameters& params, DepressionHierarchy<elev_t>& deps, ArrayPack& arp) {
  ...
  for (int d = 1; d < (int)deps.size(); d++) {
    ...
    assert(
        dep.water_vol == 0 || (dep.lchild == NO_VALUE && dep.rchild == NO_VALUE) ||
    ...
    if (dep.lchild != NO_VALUE && deps.at(dep.lchild).water_vol > dep.water_vol &&
        dep.water_vol > 0)  // fp_ge(dep.water_vol,0))
...
static void MoveWaterIntoPits(Parameters& params, DepressionHierarchy<elev_t>& deps, ArrayPack& arp) {
      ...
      for (int x = 0; x < arp.topo.width(); x++) {
        ...
        if (arp.runoff(x, y) > 0) {  // There is surface water in the cell.
          if (arp.label(x, y) ==
            ...
            arp.total_loss_to_ocean += arp.runoff(x, y) * arp.cell_area[y];
  ...
  else {  // infiltration is turned on, so we have to move water cell by cell and calculate how much will infiltrate as
          // we go.
#pragma omp parallel for default(none) shared(arp)
    ...
    Array2D<char> dependencies(arp.topo.width(), arp.topo.height(), 0);
      ...
      if (n == NO_FLOW) {
        ...
        if (arp.runoff(c) > 0) {  // if there is water available, pass it downstream.
          ...
          if (x == nx)  // same x means use e-w direction
            ...
            distance =
                (std::pow(
                    (std::pow(arp.cellsize_e_w_metres[ny], 2.0) + std::pow(params.cellsize_n_s_metres, 2.0)), 0.5)) /
                2.0;
          ...
          double my_infiltration = CalculateInfiltration(c, distance, arp.runoff(c), arp);
          arp.wtd(c) += my_infiltration / arp.porosity(c);
          ...
          if (arp.runoff(c) > 0) {
            ...
            arp.wtd(n) += my_infiltration / arp.porosity(n);
            arp.runoff(c) -= my_infiltration;
...
static void CalculateWtdVol(
  ...
  for (int y = 0; y < wtd.height(); y++)
    for (int x = 0; x < wtd.width(); x++) {
      ...
      // deps[clabel].wtd_only -= arp.cell_area[y] * arp.porosity(x,y) * arp.fdepth(x,y) *
      // (exp(arp.wtd(x,y)/arp.fdepth(x,y)) - 1);
      deps[clabel].wtd_only -=
          static_cast<double>(cell_area[y]) * static_cast<double>(porosity(x, y)) * static_cast<double>(wtd(x, y));
      // this records the space that is only in the ground - not including above-ground space. This is needed to obtain
      // the appropriate total wtd_vols for metadepressions, below.
  ...
  for (int d = 0; d < (int)deps.size(); d++) {
    ...
    dep.wtd_vol =
        dep.dep_vol +
        dep.wtd_only;  // this records the total volume, both above and below ground, available to store water.

    if (dep.lchild != NO_VALUE) {  // if it has children, it is a metadepression and we need to add the groundwater
                                   // space from the children.
      ...
      dep.wtd_only +=
          deps.at(dep.lchild).wtd_only;  // add this so that we know how much to add to the parent of this depression.
...
static inline double CalculateInfiltration(int cell, double distance, double h_0, ArrayPack& arp) {
  ...
  // assigning a small minimum vertical ksat, since otherwise in cells with zero vert_ksat, the delta_t will be
  // infinity.
  ...
  // Manning's equation is rearranged to compute the amount of time it will take the given amount of water to cross the
  // cell.
  ...
  if ((arp.wtd(cell) * static_cast<double>(arp.porosity(cell))) >
      -my_infiltration) {  // if the cell is getting saturated partway.
    my_infiltration =
        std::min(my_infiltration, static_cast<double>(-arp.wtd(cell)) * static_cast<double>(arp.porosity(cell)));
...
static void MoveWaterInDepHier(
  ...
  if (this_dep.water_vol > this_dep.wtd_vol) {
    if (current_depression == OCEAN)
      arp.total_loss_to_ocean += this_dep.water_vol - this_dep.wtd_vol;
    ...
    assert(fp_eq(this_dep.water_vol, 0) || fp_le(this_dep.water_vol, this_dep.wtd_vol));
...
static void MoveWaterInOverflow(
  ...
  for (int n = 1; n <= neighbours; n++) {  // Check out our neighbours
    ...
    if ((this_dep.dep_label == arp.label(nx, ny)) &&
        (move_to_cell == NO_VALUE || arp.topo(nx, ny) < arp.topo(move_to_cell))) {
  ...
  // because we have already done the overflow from that depression, so it could lead to an overfull depression if we
  // do. For simplicity, I am choosing not to infiltrate in the out_cell even if it shares this depression's label.
  // TODO: fix this.
  ...
  while (extra_water > 0) {
    ...
    if (x == move_to_cell_x)                               // this means use e-w direction
      ...
      distance = (std::pow(
          (std::pow(arp.cellsize_e_w_metres[move_to_cell_y], 2.0) + std::pow(params.cellsize_n_s_metres, 2.0)), 0.5));

    // We must convert extra_water to a height when calling CalculateInfiltration.
    double my_infiltration =
        CalculateInfiltration(move_to_cell, distance, (extra_water / arp.cell_area[move_to_cell_y]), arp);
    ...
    if (my_infiltration > 0) {
      ...
      if (-(arp.wtd(move_to_cell)) < (my_infiltration / static_cast<double>(arp.porosity(move_to_cell)))) {
        ...
        arp.wtd(move_to_cell) +=
            my_infiltration / static_cast<double>(arp.porosity(move_to_cell));  // infiltrate into the cell
      }

      extra_water -=
          my_infiltration *
          static_cast<double>(arp.cell_area[move_to_cell_y]);  // and that much of the water has been used up.
      ...
      // We have infiltrated into the cell and adjusted extra_water. Now, we need to adjust the wtd_vol of affected
      // depressions.
      my_label = arp.label(move_to_cell);

      while (my_label != OCEAN) {  // adjust the wtd_vol of me and all my parents

        // only if it's below the outlet elevation do we modify wtd vol; cells higher on the slope never counted towards
        // the wtd_vol to begin with.
    ...
    if (ndir == NO_FLOW) {                // we've reached a pit cell, so we can stop.
      this_dep.water_vol += extra_water;  // Add any remaining extra_water to this depression. If the depression is
                                          // overfilled, it will still overflow in OverflowInto.
      extra_water = 0;  // No need to worry about infiltration in the pit cell: this will happen down in
                        // FillDepressions.
      break;
...
static dh_label_t OverflowInto(
  ...
  if (root == stop_node || root == OCEAN) {
    ...
    if (this_dep.wtd_vol > this_dep.dep_vol && this_dep.water_vol < this_dep.wtd_vol &&
        params.infiltration_on == true) {
      ...
      if ((deps.at(previous_dep).parent == this_dep.dep_label && deps.at(previous_dep).ocean_parent == true) ||
  ...
  // I require knowledge of the depression that sent the water to MoveWaterinOverflow, so that I know where to start
  // moving the water downslope from. When using the jump table, I know where the water ends up, but how do I know which
  // side of the depression it flows in from?

  if (jump_table.count(root) != 0 &&
      params.infiltration_on == false) {  // we can only use the jump table if we dont' want water to infiltrate, or we
                                          // won't know where infiltrating water is coming from.
    if (jump_table.at(root) == OCEAN)
      arp.total_loss_to_ocean += extra_water;

    return jump_table[root] =
               OverflowInto(jump_table.at(root), -1, stop_node, deps, jump_table, extra_water, params, arp);
  }
  ...
  if (this_dep.odep != NO_VALUE) {  // Does the depression even have such a neighbour?
    ...
    if (odep.water_vol < odep.wtd_vol) {  // Can overflow depression hold more water?
      ...
      if (this_dep.geolink == OCEAN)
        arp.total_loss_to_ocean += extra_water;
      return jump_table[root] = OverflowInto(
                 this_dep.geolink, this_dep.dep_label, stop_node, deps, jump_table, extra_water, params, arp);
  ...
  if (this_dep.parent == OCEAN)
    arp.total_loss_to_ocean += extra_water;
  return jump_table[root] =
             OverflowInto(this_dep.parent, this_dep.dep_label, stop_node, deps, jump_table, extra_water, params, arp);
}
...
static void FillDepressions(
  ...
  // upwards
  flood_q.emplace(pit_cell % arp.topo.width(), pit_cell / arp.topo.width(), arp.topo(pit_cell));
  ...
  while (!flood_q.empty()) {
    ...
    //     if(water_vol - (current_volume - (arp.cell_area[c.y] * arp.porosity(c.x,c.y) * arp.fdepth(c.x,c.y) *
    //     (exp(arp.wtd(c.x,c.y)/arp.fdepth(c.x,c.y)) - 1) ) ) <= FP_ERROR){

    if ((((water_vol -
           (current_volume - (static_cast<double>(arp.cell_area[c.y]) * static_cast<double>(arp.porosity(c.x, c.y)) *
                              static_cast<double>(arp.wtd(c.x, c.y))))) <= FP_ERROR) &&
         dep_labels.count(arp.label(c.x, c.y)) == 1) ||
        water_vol - current_volume < FP_ERROR) {
      // we are checking for two similar but different cases here: either the current volume is large enough to
      // accommodate all of the water, without needing to check for groundwater space in the current cell; or there is
      // enough space if we account for groundwater space in the current cell AND the current cell is labelled as a part
      // of this depression. The distinction is necessary because the outlet cell of the depression does not always
      ...
      auto water_level = DetermineWaterLevel(
          arp.topo,
          arp.porosity,
          arp.cell_area,
          arp.wtd,
          ...
          current_volume,
          current_area,
          area_times_elevation_total);

      // Water level must be higher than (or equal to) the previous cell we looked at, but lower than (or equal to) the
      // current cell
    ...
    if (static_cast<int32_t>(arp.topo.xyToI(c.x, c.y)) != out_cell) {
      ...
      //  depth-variable porosity version
      //   water_vol += arp.cell_area[c.y] * arp.porosity(c.x,c.y) * arp.fdepth(c.x,c.y) *
      //   (exp(arp.wtd(c.x,c.y)/arp.fdepth(c.x,c.y)) - 1);
      water_vol += static_cast<double>(arp.cell_area[c.y]) * static_cast<double>(arp.porosity(c.x, c.y)) *
                   static_cast<double>(arp.wtd(c.x, c.y));
