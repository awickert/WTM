...
*Regenerated by every `tests/run_all.sh`. Each cell counts the runs that exercised that combination: **0** marks a combination nothing tests, `n/a` one the model refuses by design.*

**144 runs** across **30 tests**. **16** distinct (solver, collector, integrator) combinations are exercised; of those **4** run in both equilibrium and transient and **12** are equilibrium-only. Full detail, every pairwise crossing and the gap list: [`tests/COVERAGE.md`](tests/COVERAGE.md).
