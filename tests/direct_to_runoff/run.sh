...
TOL="${TOL:-1e-4}"          # metres; settled if final per-cycle |Δwtd| below this, and |wtd| below this = at surface
...
emit gathered
"$WTM" "$WORK/gathered.cfg" -wtm_anderson -wtm_direct_to_runoff -wtm_eq_tol 0 > "$WORK/gathered.log" 2>&1 \
  || { echo "RUN FAILED: gathered"; tail -3 "$WORK/gathered.log"; exit 2; }
emit piled
"$WTM" "$WORK/piled.cfg" -wtm_anderson -wtm_dev_allow_aboveground_water_columns -wtm_eq_tol 0 > "$WORK/piled.log" 2>&1 \
...
TOL="$TOL" MB_TOL="$MB_TOL" PILE_MIN="$PILE_MIN" GSETTLE="$gsettle" \
...
above = float(gat.max()); below_ok = bool((gat <= tol).all())
at_surface = bool(abs(above) <= tol)            # gathered: table held exactly at the surface (seepage face)
...
print(f"  GATHERING      : gathered max wtd = {above:.3e} m (=0 at surface), all wtd<=0: {below_ok}")
