...
# Refusals are LEARNED, not listed. tests/combination_sweep attempts every solver x integrator x
# collector at both run types and records what each one did, so a cell can be marked from OBSERVATION:
#   kind=design  the model threw a deliberate, explanatory error -- a real incompatibility
#   kind=hard    the solver did not converge, even after the sweep retried at dt/8
# This replaced a hand-written list of "combinations refused by design". That list was an opinion, and
# it was wrong: the first sweep run reported 64 design refusals, of which 56 were a missing input in
# the sweep's own fixture. Attempting a combination is evidence; asserting it is forbidden is not.
def parse_refusals(path):
    """{(axis=value, axis=value): kind} for every pair inside each recorded refusal."""
    out = {}
    try:
        f = open(path)
    ...
    with f:
        for line in f:
            if not line.startswith("refusal "):
                continue
            toks = dict(t.split("=", 1) for t in line.split()[1:] if "=" in t and t.split("=", 1)[0] != "msg")
            kind = toks.pop("kind", "design")
            items = [f"{k}={v}" for k, v in toks.items()]
            for i in range(len(items)):
                for j in range(len(items)):
                    if i != j:
                        out.setdefault((items[i], items[j]), kind)
    return out


# The ONE structural unreachability that cannot present as a refusal, because the model does not
# refuse it -- it silently resolves it away. active_set requires a b=0 residual path, so it
# auto-enables volume storage; requesting secant backward-Euler with active_set therefore yields
# be_volume. This is not a curated opinion: the fingerprints show it directly, since every active_set
# run reports integrator=be_volume no matter what its config asked for.
STRUCTURAL = {("integrator=be_secant", "collector=active_set"): "design"}

REFUSED = {}   # filled in main() from the same log, plus STRUCTURAL
...
def crossing_table(runs, a, b):
    ...
    for x in va:
        ...
        for y in vb:
            ...
            if n:
                cells.append(str(n))
            elif REFUSED.get((f"{a}={x}", f"{b}={y}")) or REFUSED.get((f"{b}={y}", f"{a}={x}")):
                k = REFUSED.get((f"{a}={x}", f"{b}={y}")) or REFUSED.get((f"{b}={y}", f"{a}={x}"))
                cells.append("n/a" if k == "design" else "hard")
            else:
                cells.append("**0**")
...
def write_readme(path, runs, gaps):
    ...
    B.append(f"*Regenerated by every `tests/run_all.sh`. Each cell counts the runs that exercised that "
             ...
             f"design, `hard` one that does not converge. The last two are observed by "
             f"`tests/combination_sweep`, not asserted.*")
    B.append("")
...
def main():
    ...
    if not runs:
        ...
        return 2
    REFUSED.update(STRUCTURAL)
    REFUSED.update(parse_refusals(log))
    ...
    w("A blank cell is a combination **no run exercises**. `by design` and `does not converge` are "
      "LEARNED from tests/combination_sweep, which attempts every combination and records what it did "
      "-- so those are observations, not assertions.\n")
    gaps = []
        ...
        for x in va:
            ...
            for y in vb:
                ...
                if n:
                    ...
                    why = REFUSED.get((f"{a}={x}", f"{b}={y}")) or REFUSED.get((f"{b}={y}", f"{a}={x}"))
                    if why:
                        row.append("by design" if why == "design" else "does not converge")
                    else:
                        row.append(" ")
