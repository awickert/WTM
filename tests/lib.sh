...
_wtm_vacuity_check() { # $1 = suite tag ; reports always, returns 1 only for a NON-exempt suite
    local tag="$1" tests_dir out rc=0
    tests_dir=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
    [ -d "${WORK:-}" ] || return 0
    command -v python3 >/dev/null 2>&1 || return 0
    out=$("$tests_dir/nonvacuous.py" --summary "$WORK" 2>/dev/null)
    [ -n "$out" ] || return 0
    case " $WTM_VACUITY_EXEMPT " in
        *" $tag "*) echo "  note  STRUCTURE $tag is structurally exempt (see lib.sh): $out" ;;
        *)
            if printf '%s' "$out" | command grep -q "no structure"; then
                echo "  FAIL  STRUCTURE $tag: $out -- it compared fields that assert nothing:" >&2
                "$tests_dir/nonvacuous.py" --report "$WORK" >&2
                rc=1
            else
                echo "  OK   STRUCTURE $out"
            fi ;;
    esac
    return $rc
}
