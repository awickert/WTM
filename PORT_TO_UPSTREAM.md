...
| **Wider storativity surface-smoothing (recommended per-run) `-wtm_storativity_surface_smoothing_width 0.2`** | **Speed (quantified).** The wtd=0 storativity jump drives the frozen-coefficient contraction failure ("disease 2", distinct from the deep singularity "disease 1" = the bedrock floor). Widening 0.01→~0.2 m, composed with `-wtm_Tbar`, cuts cold-start iterations **~16–24% for BOTH Anderson and Picard** (island AND 384k Esquibel); lifts Picard's island ceiling 1→2 wk. Does **not** raise Anderson's step ceiling. `benchmark/SURFACE_SMOOTHING_STABILIZATION.md`. | **Slightly** — shifts the shallow-cell equilibrium ~0.2–0.35% (why the DEFAULT stays 0.01 m). | Flag exists (default 0.01 m); recommendation documented at the knob + benchmark doc. | ✅ recommend as a **per-run** setting; do **NOT** change the default upstream (preserves v2.0.1 numbers). |
