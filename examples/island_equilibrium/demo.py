...
def time_step_block(mode, dt):
    ...
    if mode == "adaptive":
        return (f"    mode: adaptive\n"
                ...
                f"                      # MODFLOW 6 and ParFlow, which make you state the controller's\n"
                f"                      # floor rather than guessing one. DERIVED from dt (1e-5 x, their\n"
                f"                      # recommended DTMIN) so it stays five orders below the step at\n"
                f"                      # every --dt-weeks this demo is run with: the 512 wk case exists\n"
                f"                      # to show adaptive SUBDIVIDING where fixed dies, and a floor that\n"
                f"                      # did not scale would cap the subdivision being demonstrated\n"
