...
def time_step_block(mode, dt):
    ...
    if mode == "adaptive":
        return (f"    mode: adaptive\n"
                ...
                f"    dt_min: \"{dt * 1e-5:g}s\"   # REQUIRED beside an explicit mode: adaptive -- WTM follows\n"
                f"                      # MODFLOW 6 and ParFlow in making you STATE the controller's floor\n"
                f"                      # rather than guessing one. (Their 1e-5 is an absolute length for\n"
                f"                      # a reporting reason; the 1e-5 x dt RATIO is WTM's own choice, so\n"
                f"                      # the margin holds at any --dt-weeks. See BDF2_ADAPTIVE_DESIGN.md.)\n"
                f"                      # It does not bind here and was not expected to: the smallest step\n"
                f"                      # ever taken on corsica --equilibrium is 1.7117e+05 s, 542x above\n"
                f"                      # even a fixed 315.36 s, and no corsica log has ever clamped\n"
