"""11x11 block of the REAL corsica terrain, outer ring held fixed, interior 3x3 integrated.
...
R0, C0 = 115, 58                          # top-left of the 5x5
Z  = topo[R0:R0+11, C0:C0+11]
FD = np.maximum(200.0/(1+150*slope[R0:R0+11, C0:C0+11]), 2.0)
K  = ks[R0:R0+11, C0:C0+11]
Hbar = st[:, R0:R0+11, C0:C0+11].mean(axis=0)     # time-mean, used for the fixed ring
H0   = st[0,  R0:R0+11, C0:C0+11].copy()          # initial condition = the real restart state
...
INT = [(i, j) for i in range(1, 10) for j in range(1, 10)]
def rhs(h):
...
def run(years, dt_yr, removal, h_init):
    ...
    for n in range(int(years/dt_yr)):
        ...
        h[0,:] = H0[0,:]*0 + Hbar[0,:]; h[10,:] = Hbar[10,:]      # ring stays fixed
        h[:,0] = Hbar[:,0]; h[:,10] = Hbar[:,10]
        if removal: h = np.minimum(h, 0.0)
        out.append((n*dt_yr, h[5,5], h[6,5]))                    # A=(120,63)->(5,5)  B=(121,63)->(6,5)
    return np.array(out)
for removal in (True, False):
    r = run(900, 0.02, removal, H0)
    tail = r[len(r)//2:]
    print(f"=== 11x11 block, removal {'ON ' if removal else 'OFF'} ===")
    print("    t(yr)      h_A       h_B")
