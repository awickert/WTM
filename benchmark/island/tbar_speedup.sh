...
echo "cores=$N  domain=island(8775)  fsm=off  eq_tol=default(0.01m)  report_interval=50"
...
for cfg in base tbar trbdf2 both; do
  for mult in 1 2 4 8 16 32; do
    ...
    sed "s#^deltat.*#deltat $dt#;s#^fsm_on.*#fsm_on 0#;s#^total_cycles.*#total_cycles 300#;s#^save_nreport_interval.*#save_nreport_interval 1#;s#^textfilename.*#textfilename ${stem}.txt#;s#^outfile_prefix.*#outfile_prefix ${stem}_#" eq_awickert.cfg > "$stem.cfg"
    t0=$(date +%s.%N)
